...
async function summarizeWithLLM(paper, options = {}) {
  if (!paper.abstract?.trim()) throw new Error('Missing source abstract for Korean summary');
  ...
  try {
    const res = await fetcher(OPENCODE_GO_URL, {
      ...
      headers: {
        ...
        'User-Agent': 'Study-ML-paper-collector/1.0',
    ...
    if (!res.ok) {
      ...
      // Keep useful error codes without logging arbitrary provider response text.
      const suffix = typeof type === 'string' && /^[A-Za-z0-9_-]{1,80}$/.test(type) ? ` (${type})` : '';
      throw new Error(`LLM API ${res.status}${suffix}`);
    }
...
async function collectPapers(options = {}) {
  ...
  // Restored citation records have no abstract; fetch the source before retrying.
  const missingAbstracts = sorted.filter(p => !normalizeSummary(p) && !p.abstract?.trim());
  if (USE_LLM && missingAbstracts.length) {
    try {
      const sources = new Map((await fetchEntriesByIds(missingAbstracts.map(p => p.id)))
        .map(paperFromEntry).filter(Boolean).map(p => [p.id, p.abstract]));
      for (const p of missingAbstracts) p.abstract = sources.get(p.id) || '';
    } catch (error) {
      console.warn(`Could not restore source abstracts: ${error.message}`);
    }
  }
  ...
  for (const p of sorted) {
    ...
    if (USE_LLM) {
      try {
        ...
        console.warn(`LLM summary failed for ${p.id}: ${err.message}; summary unavailable`);
...
function serialize(papers, extraMetadata = {}) {
  const available = papers.filter(p => normalizeSummary(p)).length;
  const unavailable = papers.length - available;
  ...
  const metadata = JSON.stringify({
    ...
    summarizer: unavailable ? (available ? 'mixed' : 'unavailable') : (available ? 'llm' : 'unavailable'),
    recommendationVersion: 1,
    ...extraMetadata,
    summary: { total: papers.length, available, unavailable, status: unavailable ? (available ? 'partial' : 'failed') : (available ? 'ok' : 'empty') },
  }, null, 2);
...
async function main() {
  if (process.argv.includes('--self-test')) {
    ...
    assert.deepEqual(
      await summarizeWithLLM({ title: 'Success test', authors: 'Test', categories: ['cs.CL'], abstract: 'Test abstract' }, {
        ...
        fetcher: async (_url, request) => {
          assert.equal(request.headers['User-Agent'], 'Study-ML-paper-collector/1.0');
    ...
    assert.equal(normalizeSummary(unavailableSummary), null);
    assert.equal(reusableSummary(unavailableSummary), null);
    assert.equal(normalizeSummary({ ...summaryFixture, summaryKo: 'English-only summary' }), null);
    await assert.rejects(summarizeWithLLM({ ...summaryFixture, title: 'No abstract' }), /Missing source abstract/);
    await assert.rejects(summarizeWithLLM({ title: 'API error', authors: 'Test', categories: ['cs.CL'], abstract: 'Source abstract' }, {
      fetcher: async () => ({ ok: false, status: 400, json: async () => ({ error: { type: 'MissingSessionID', message: 'private response text' } }) }),
    }), error => error.message === 'LLM API 400 (MissingSessionID)');
  ...
  const unavailable = papers.filter(p => !normalizeSummary(p)).length;
  console.error(`Korean summaries: ${papers.length - unavailable}/${papers.length} available; ${unavailable} unavailable`);
  if (process.argv.includes('--require-summaries') && (unavailable || papers.length === 0)) {
    throw new Error('Required Korean summaries are incomplete; existing data was not overwritten');
  }
